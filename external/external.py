...
def send_discord_message(discordMessage):
    message = {"content": discordMessage}
...
def test_tcp_connection(host, port):
    ...
    try:
        ...
        send_discord_message(
            M_INSTANCE + ": TCP Connection refused, server not available"
        )
        return -1
...
async def metrics():
    ...
    try:
        ...
        async with websockets.connect(
            "ws://" + TEST_HOST + ":" + str(TEST_HOST_PORT) + "/latency-test"
        ) as websocket:
