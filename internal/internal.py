...
async def redis_test(
    redishost: str, portnumber: int, keyname: str, keyvalue: str
) -> dict:
...
async def latency_test(websocket: WebSocket):
    ...
    if data == "8d5c8261-07b1-49b3-96e3-1d8bdfb98234":
        redis_time = await redis_test(
            os.environ.get("REDIS_HOST", "127.0.0.1"),
            int(os.environ.get("REDIS_PORT", 6379)),
            "LATENCYTEST",
            "testRedis",
        )
        nats_time = await nats_test(
            os.environ.get("NATS_HOST", "127.0.0.1"),
            os.environ.get("NATS_PORT", "4222"),
            "LATENCYTEST",
            "testNats",
        )
        portgres_time = await postgres_test()
        _rDict = {
            "redistime": redis_time,
            "natstime": nats_time,
            "postgrestime": portgres_time,
        }
