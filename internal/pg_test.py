...
def create_tables():
    ...
    commands = (
        "CREATE TABLE nwltable (first_char character varying(128) NOT NULL, second_char character varying(64), first_time timestamp without time zone DEFAULT now(), first_json jsonb, second_json jsonb)",
    )
    try:
        config = load_config()
        with psycopg2.connect(**config) as conn:
...
def get_data():
    ...
    try:
        with psycopg2.connect(**config) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT first_time FROM nwltable ORDER BY first_time DESC LIMIT 1;"
                )
                row = cur.fetchone()
        return row
    except (Exception, psycopg2.DatabaseError) as error:
...
def run():
    ...
    # Inserting data returning field is first_time
    if insert_data(mylist) == current_time:
        endTimeInsert = time.perf_counter()
        # Selecting first_time and cheking with current_time
        if get_data()[0] == current_time:
            endTimeSelect = time.perf_counter()
            # Deleting a row based on current time and check affected rows
            if delete_data(current_time) == 1:
                endTime = time.perf_counter()
                return {
                    "totalTime": endTime - startTime,
                    "selectTime": endTimeSelect - endTimeInsert,
                    "insertTime": endTimeInsert - startTime,
                    "deleteTime": endTime - endTimeSelect,
                }
            else:
                return -1
        else:
